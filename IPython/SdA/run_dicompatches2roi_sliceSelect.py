...
if __name__ == '__main__':
    ...
    side = fileline[4]
    typeEnh = fileline[5]
    centroid_slice = int(fileline[-1][:-1]) -1
    centroid_y = int(fileline[-2][:-1]) -1
    centroid_x = int(fileline[-3][1:-1]) -1
    
    print '\n%%%%%%%%%%%%%%%%%%%%%% Centroid Loc and slice = %s \n%%%%%%%%%%%%%%%%%%%%%%' % str( [centroid_slice, centroid_y, centroid_x] )
    ...
    alltestpatches = extrP.extractestPatches_sliceSelect(slicestime, ha, wa, stride, 'test', [centroid_slice, centroid_y, centroid_x])
