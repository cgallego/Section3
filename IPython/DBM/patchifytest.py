...
class Patchifytest(object):
    ...
    def extractestPatches(self, slicestime, ha, wa, stride, name, bestmodelname, tempsize):
        """
        Currently only supports rectangular patches
        ...
        """        
        self.ptcsize = [ha,wa]
        with open(bestmodelname, 'rb') as fp:
            bestmodel = pickle.load(fp)
        ...
        for k in range(nslices):
            ...
            for i in range(ncols/stride):
                for j in range(nrows/stride): # npatches =17*17=289
                    ...
                    stridepatches = []
                    ## for the post-contract imgs
                    for l in range(len(normimgslicestime)): 
                        
                        # extract patch inside the rectangular ROI
                        patchk = normimgslicestime[l][k,x1:x2,y1:y2]
            ...
            imgX = Xtmp.reshape( Xtmp.shape[0], tempsize, 100)[:,3,:]
            ...
            for i in range(ncols/stride):
                for j in range(nrows/stride):
                    if(npatches_holder[c]):
                        ...
                        ###############
                        # in train
                        predtrain = bestmodel.predict_functions(patch)
            ...
            for i in range(ncols/stride):
                for j in range(nrows/stride): # npatches =17*17=289
                    ...
                    if(flag_output[i,j]):
                        ...
                        clasfslicepatches.append( np.zeros(tempsize*10*10) )
