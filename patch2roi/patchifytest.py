...
class Patchifytest(object):
    ...
    def extractestPatches_cascade(self, slicestime, ha, wa, stride, centroid_s,centoroid_y,centoroid_x, diag1, diag2, output_folder):
        """
        Currently only supports rectangular patches
        extractPatch takes pimg0, pimg1, pimg2, pimg3, pimg4 vol arrays and a list of selected slices, 
        selected x,y init patch locations
        ...
        alltestpatches = []  
        alltest_matchedtolesion = []
        
        positive_patches = []
        label_cascadepatches = []
        all_sumofpositives = 0
        
        fig, ax = plt.subplots(nrows=int(np.sqrt(nslices)), ncols=int(np.sqrt(nslices))+2, figsize=(12, 12))
        axes=ax.flatten()

        for k in range(centroid_s-2,centroid_s+2):
            allslicepatches = []
            npatches_holder = []
            allslicepatches_matchedtolesion = []
            ...
            for i in range(ncols/stride):
                for j in range(nrows/stride): # npatches =17*17=289
                    ...
                    # check whether inside of lesion:
                    if(int(centroid_s[0]) == k):
                        indiag1 = x1 >= np.min(np.asarray(diag1)) and y1 <= np.max(np.asarray(diag1))
                        indiag2 = x2 >= np.min(np.asarray(diag2)) and y2 <= np.max(np.asarray(diag2))
                        if(indiag1 or indiag2):
                            allslicepatches_matchedtolesion.append(1)
                        else:
                            allslicepatches_matchedtolesion.append(0)                            
                    if(int(centroid_s[0]) != k):
                        allslicepatches_matchedtolesion.append(0)
                                
                            
            alltest_matchedtolesion.append( allslicepatches_matchedtolesion )
            alltestpatches.append(allslicepatches) 
            ...
            for i in range(ncols/stride):
                for j in range(nrows/stride):
                    if(npatches_holder[c]):
                        ...
                        # only retain positive
                        if(predtrain.argmax(0) == 1):
                            positive_patches.append(patch)
                            label_cascadepatches.append(allslicepatches_matchedtolesion[cp])
                        # augment one for patch
                        cp+=1
                    # augment one for flag
                    c+=1
            ...
            # show       
            axes[k].imshow( clasfpathcesimgX, cmap=plt.cm.gray )
            axes[k].set_axis_off()
            axes[k].set_adjustable('box-forced')
            
            if(int(centroid_s[0]) == k):
                axes[k].annotate(k, xy=(500, 500), xytext=(512, 512),
                            arrowprops=dict(facecolor='red', shrink=0.05))
                        
            print "done plotting positives = %i" % sumofpositives
            all_sumofpositives = all_sumofpositives + sumofpositives
       
            ######################################################
            # Plot slice only
            ######################################################
            if( k == int(centroid_s[0])):
                figSlice, axSlice = plt.subplots(nrows=1, ncols=3, figsize=(12, 12))
                axesSlice=axSlice.flatten()
                ...
                # only display slice
                axesSlice[0].imshow( pathcesimgX, cmap=plt.cm.gray )
                axesSlice[0].set_axis_off()
                axesSlice[0].set_adjustable('box-forced')
                
              
                axesSlice[1].imshow(clasfpathcesimgX, cmap=plt.cm.gray)
                axesSlice[1].set_axis_off()
                axesSlice[1].set_adjustable('box-forced')   
                axesSlice[1].add_artist(Circle(xy=([centoroid_x[0],centoroid_y[0]]), radius=np.mean(diag1-diag2).max()/2, alpha=0.4))

                # Display normalized histogram
                axesSlice[2].imshow( flag_output, cmap=plt.cm.gray)
                axesSlice[2].set_axis_off()
                axesSlice[2].set_adjustable('box-forced')   
                axesSlice[2].add_artist(Circle(xy=([centoroid_x[0],centoroid_y[0]]), radius=np.mean(diag1-diag2).max()/2, alpha=0.4))

        # plot in big screen
        figSlice.tight_layout()
        plt.title('slice_'+str(centroid_s[0])+'_centoroid_xy_'+str(centoroid_x[0])+'_'+str(centoroid_y[0]), fontsize=14)   
        print "done ALL positives = %i" % all_sumofpositives

        return positive_patches, label_cascadepatches         
